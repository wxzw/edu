# 老师端小程序分阶段实现方案与代码设计思路

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 基于现有 Spring Boot + PostgreSQL 后端架构，实现老师端小程序完整功能，复用现有数据库和认证体系。

**Architecture:** 采用与学生端平行的 `com.community.edu.teacher` 包结构，通过 `TeacherScopeService` 解析老师身份，所有查询以 `teacherId` + `campusId` 过滤。前端采用 `pages/teacher/*` 页面组织，与学生端 `pages/student/*` 对称。

**Tech Stack:** Spring Boot 3.3 + PostgreSQL + MyBatis-Plus + JWT + uni-app (Vue 3)
...
**验收标准：**
- [x] `TeacherScopeService` 能正确解析老师身份
- [x] 非老师身份访问 `/api/teacher/*` 返回 403
- [x] Controller 骨架能响应测试请求
...
**验收标准：**
- [x] 老师身份登录后首页展示今日课程卡片（时间、班级、主题、教室）
- [x] 待考勤、待点评数字准确统计
- [x] 数据按当前老师 ID 和校区 ID 隔离
- [x] 下拉刷新更新数据
...
**验收标准：**
- [x] 老师只能查看自己任课或担任班主任的班级
- [x] 学生列表展示姓名、剩余课时、最近考勤状态
- [x] 学生档案课时数据与数据库一致
- [x] 家长联系方式脱敏展示
...
**验收标准：**
- [x] 考勤确认后自动计算应扣课时
- [x] 余额不足时给出提示但不阻断（标记为欠费）
- [x] 写入课时流水记录，包含操作人、操作前/后余额
- [x] 支持查看某学生的课时流水
